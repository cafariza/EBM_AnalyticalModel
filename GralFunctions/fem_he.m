...
for g=1:3 
subplot(g,1,3)
plot(Unum{1,g}(:,1),x_hm,'k-o', ModeShapes_an(:,g),x_hm, 'b--x') ; grid on; hold on;
for i=1:count
plot(savemodes_he{1,i}(:,g)*-1,x_efem, 'r--o','color',C{i}); hold on; 
end
end

xlabel('Displacement U','FontSize',12); ylabel('x');
legend(': Numerical', ': FEM', ': Analytical');
title(' Modal Shapes ');
