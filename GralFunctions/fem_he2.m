...
savex_he={};
count=0;
C = {'b','m','c','y','g',[.5 .6 .7],[.8 .2 .6]}; % Cell array of colors.
Nmin=3;
Nmax=100;
iterval=[Nmin,N,Nmax];
for i=1:1:length(iterval)
     N_elem=iterval(i);
     count=count+1;
...
for g=1:3 
figure (g);  
plot(ModeShapes_cast(:,g),x_hm,'k-', ModeShapes_an(:,g),x_hm, 'r-', 'LineWidth', 2) ; grid on; hold on;
for i=1:count
plot(savemodes_he{1,i}(:,g),savex_he{1,i}(1,:)', '-.','color',C{i},'LineWidth', 2 ); hold on; 
end
xlabel('Transverse Displacement U','FontSize',12); ylabel('x');
legend(': Numerical', ': Analytical', ': FEM_{he_{min}}',': FEM_{he=l_m}',': FEM_{he_{max}}');
str = sprintf('   Mode %d',g);
title(str);
end
